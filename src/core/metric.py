...
def speed_metric(func):
    """Декоратор для измерения времени выполнения синхронной функции.

    Этот декоратор оборачивает синхронную функцию и логирует время, затраченное на ее выполнение.

    Args:
        func (Callable): Cинхронная функция, время выполнения которой нужно измерить.

    Returns:
        Callable: Обернутая синхронная функция с логированием времени выполнения.
    """

    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        """Обертка для синхронной функции, измеряющая время выполнения.
